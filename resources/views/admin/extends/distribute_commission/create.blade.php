    ...
    <div class="max-w-4xl mx-auto">
        ...
        <div class="mb-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
                ...
                <p class="text-sm text-gray-500 mt-1 ml-13">Add a new commission distribution</p>
            </div>
        ...
        <div class="bg-white/80 rounded-2xl shadow-lg shadow-indigo-100/50 border border-white/50 overflow-hidden">
            <div class="p-5 border-b border-gray-100 flex items-center gap-2">
                ...
                <span class="font-semibold text-gray-700">Commission Information</span>
            </div>
            <div class="p-6">
                <form action="{{ route('admin.distribute-commission.create') }}" method="POST">
                    @csrf
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="flex flex-col">
                            ...
                            <select name="commission_setting_id"
                                ...
                                <option value="{{ $type->id }}" {{ old('commission_setting_id') == $type->id ? 'selected' : '' }}>{{ $type->name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="flex flex-col">
                            ...
                            <div class="relative">
                                <input type="number" step="0.01" min="0" max="100" name="commission_percentage" value="{{ old('commission_percentage') }}"
                                       class="form-input !rounded-xl !border-gray-200 !bg-gray-50 focus:!bg-white focus:!border-indigo-400 focus:!ring focus:!ring-indigo-200/50 !transition-all"
                                       placeholder="Enter percentage (e.g. 10.50)">
                            </div>
