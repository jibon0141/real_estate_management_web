...
<div class="min-h-screen p-4 sm:p-6 lg:p-8" style="background: linear-gradient(135deg, #f0f4ff 0%, #e8ecf8 100%);">
    <div class="max-w-4xl mx-auto space-y-6">
        ...
        <div class="mb-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
                <h3 class="text-2xl font-bold text-gray-800 flex items-center gap-3">
                    ...
                    <i class="text-gray-400 text-lg mr-2"></i> Cash Sale
                </h3>
                <p class="text-sm text-gray-500 mt-1 ml-13">Review package details and complete cash sale</p>
            </div>
            ...
            <div class="p-6">
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    <div class="bg-gray-50 rounded-xl p-4">
                        ...
                        <p class="text-lg font-bold text-gray-800 mt-1">৳{{ number_format($package->booking_money, 2) }}</p>
                    </div>
                    <div class="bg-gray-50 rounded-xl p-4">
                    ...
                    <div class="bg-gray-50 rounded-xl p-4">
                        ...
                        <p class="text-lg font-bold text-rose-600 mt-1" id="total_sold_display">{{ number_format($totalSold, 2) }}</p>
                    </div>
                    <div class="bg-gray-50 rounded-xl p-4">
        ...
        <div class="bg-white/80 rounded-2xl shadow-lg shadow-indigo-100/50 border border-white/50 overflow-hidden">
            <div class="p-5 border-b border-gray-100 flex items-center gap-2">
                <i class="fa fa-shopping-bag text-emerald-500"></i>
                <span class="font-semibold text-gray-700">Purchase Quantity</span>
            </div>
            <div class="p-6">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
                    <div>
                        <label for="share_qty" class="block text-sm font-medium text-gray-600 mb-2">Number of Shares</label>
                        <div class="flex items-center gap-2">
                            ...
                            <button type="button" id="qty_plus" class="w-10 h-10 rounded-xl bg-emerald-100 hover:bg-emerald-200 border border-emerald-200 flex items-center justify-center text-emerald-600 font-bold transition-all">
        ...
        <div class="bg-white/80 rounded-2xl shadow-lg shadow-indigo-100/50 border border-white/50 overflow-hidden">
            <div class="p-5 border-b border-gray-100 flex items-center gap-2">
                <i class="fa fa-exchange-alt text-emerald-500"></i>
                <span class="font-semibold text-gray-700">Return Preference</span>
            </div>
            <div class="p-6">
                <form action="{{ route('admin.sell.cash.store') }}" method="POST" id="cashSaleForm">
                    @csrf
                    <input type="hidden" name="package_id" value="{{ $package->id }}">
                    <input type="hidden" name="share_qty" id="share_qty_hidden" value="1">
                    <div class="mb-6">
                        <label class="block text-sm font-medium text-gray-700 mb-3">Will you take a return?</label>
                        <div class="flex gap-4">
                            <label class="relative flex items-center gap-3 p-4 rounded-xl border-2 border-gray-200 bg-white cursor-pointer transition-all hover:border-emerald-300 has-[:checked]:border-emerald-500 has-[:checked]:bg-emerald-50 flex-1">
                                <input type="radio" name="take_return" value="1" class="peer absolute opacity-0">
                                <div class="w-5 h-5 rounded-full border-2 border-gray-300 peer-checked:border-emerald-500 peer-checked:bg-emerald-500 flex items-center justify-center transition-all">
                                    <div class="w-2 h-2 rounded-full bg-white scale-0 peer-checked:scale-100 transition-all"></div>
                                </div>
                                <div>
                                    <span class="font-semibold text-gray-800">Yes</span>
                                    <p class="text-xs text-gray-400">I want to take a return</p>
                                </div>
                            </label>
                            <label class="relative flex items-center gap-3 p-4 rounded-xl border-2 border-gray-200 bg-white cursor-pointer transition-all hover:border-red-300 has-[:checked]:border-red-500 has-[:checked]:bg-red-50 flex-1">
                                <input type="radio" name="take_return" value="0" class="peer absolute opacity-0">
                                <div class="w-5 h-5 rounded-full border-2 border-gray-300 peer-checked:border-red-500 peer-checked:bg-red-500 flex items-center justify-center transition-all">
                                    <div class="w-2 h-2 rounded-full bg-white scale-0 peer-checked:scale-100 transition-all"></div>
                                </div>
                                <div>
                                    <span class="font-semibold text-gray-800">No</span>
                                    <p class="text-xs text-gray-400">I do not want a return</p>
                                </div>
                            </label>
                        </div>
                    </div>
                    <button type="button" id="reviewSaleBtn" class="w-full py-3 text-sm font-semibold text-white bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-600 hover:to-teal-700 rounded-xl shadow-lg shadow-emerald-200 transition-all duration-200 hover:shadow-xl hover:-translate-y-0.5">
                            ...
                            <div class="bg-white rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden">
                                <div class="p-5 border-b border-gray-100 flex items-center justify-between">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span class="font-semibold text-lg text-gray-800">Confirm Cash Sale</span>
                                    </div>
                                ...
                                <div class="p-6 space-y-5">
                                    <div class="bg-gray-50 rounded-xl p-4">
                                        ...
                                        <div class="grid grid-cols-2 gap-3 text-sm">
                                            <div><span class="text-gray-400">Package:</span> <span class="font-semibold text-gray-800" id="modalPackage">{{ $package->package_name }}</span></div>
                                            ...
                                            <div><span class="text-gray-400">Return:</span> <span class="font-semibold text-gray-800" id="modalReturn">Yes</span></div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div>
                                            ...
                                            <select name="user_id" id="user_id" class="form-input w-full rounded-xl border-gray-200 bg-gray-50 focus:bg-white focus:border-indigo-400 focus:ring focus:ring-indigo-200/50 transition-all" required>
                                        ...
                                        <div>
                                            ...
                                            <select name="account_id" id="account_id" class="form-input w-full rounded-xl border-gray-200 bg-gray-50 focus:bg-white focus:border-indigo-400 focus:ring focus:ring-indigo-200/50 transition-all" required>
                                    ...
                                    <div>
                                        ...
                                        <input type="password" name="admin_password" id="admin_password" placeholder="Enter your password to confirm"
                                               class="form-input w-full rounded-xl border-gray-200 bg-gray-50 focus:bg-white focus:border-indigo-400 focus:ring focus:ring-indigo-200/50 transition-all" required>
                                    </div>
                                    ...
                                    <div class="flex gap-3 pt-2">
                                        ...
                                        <button type="submit" class="flex-1 py-2.5 text-sm font-semibold text-white bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-600 hover:to-teal-700 rounded-xl shadow-lg shadow-emerald-200 transition-all duration-200 hover:shadow-xl hover:-translate-y-0.5">
...
<script>
    $(document).ready(function () {
        ...
        function getMaxQty(takeReturn) {
            var divisor = shareCount;
            if (takeReturn === '0') {
                divisor += {{ $package->extra_benefit ?? 0 }};
            }
            return Math.max(0, Math.floor((allottedShare - totalSold - totalBenefitTaken) / divisor));
        }

        function updateDisplay() {
            var qty = parseInt($('#share_qty').val()) || 1;
            var takeReturn = $('input[name="take_return"]:checked').val();
            $('#share_qty_hidden').val(qty);

            var extraBenefit = {{ $package->extra_benefit ?? 0 }};
            var returnAmount = {{ $package->return_amount }};
            var totalLand = shareCount * qty;

            if (takeReturn === '0') {
                totalLand += extraBenefit * qty;
            }
        ...
        $('input[name="take_return"]').on('change', function () {
            var qty = parseInt($('#share_qty').val()) || 1;
            var takeReturn = $(this).val();
            var maxQty = getMaxQty(takeReturn);
            if (qty > maxQty) {
                qty = maxQty || 1;
                $('#share_qty').val(qty);
                Swal.fire({
                    icon: 'warning',
                    title: 'Limit Reached',
                    text: 'Reduced to ' + qty + '. Available share is ' + (allottedShare - totalSold - totalBenefitTaken).toFixed(2),
        ...
        $('#qty_plus').on('click', function () {
            ...
            if (qty < maxQty) {
                ...
                Swal.fire({
                    icon: 'warning',
                    title: 'Limit Reached',
                    text: 'You cannot exceed available share of ' + (allottedShare - totalSold - totalBenefitTaken).toFixed(2),
                    confirmButtonColor: '#10b981',
                    confirmButtonText: 'OK'
                });
            }
        ...
        $('#share_qty').on('input', updateDisplay);

        $('#reviewSaleBtn').on('click', function () {
            var qty = parseInt($('#share_qty').val()) || 0;
            if (qty < 1) {
                Swal.fire({ icon: 'warning', title: 'Invalid Quantity', text: 'Please select at least 1 quantity.', confirmButtonColor: '#10b981' });
                return;
            }
            var takeReturn = $('input[name="take_return"]:checked').val();
            if (takeReturn === undefined) {
                Swal.fire({ icon: 'warning', title: 'Select Return Preference', text: 'Please select Yes or No for return.', confirmButtonColor: '#10b981' });
                return;
            }
            var extraBenefit = {{ $package->extra_benefit ?? 0 }};
            var totalLand = shareCount * qty;
            var returnLabel = takeReturn === '0' ? 'No (Extra Benefit: ' + extraBenefit.toFixed(2) + ' per share)' : 'Yes';
            if (takeReturn === '0') {
                totalLand += extraBenefit * qty;
            }
            var totalPrice = unitPrice * qty;
            ...
            $('#modalReturn').text(returnLabel);
            $('#confirmModal').removeClass('hidden');
        });

        $('#closeModal, #cancelSale').on('click', function () {
