...
@section('app_styles')
<style>
    #commissionSettingTable tbody tr:nth-child(even) { background-color: #f8fafc; }
    #commissionSettingTable tbody tr:hover { background-color: #eef2ff; }
</style>
@endsection
@section('content')
    ...
    <div class="max-w-4xl mx-auto">
        ...
        <div class="mb-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
                ...
                <p class="text-sm text-gray-500 mt-1 ml-13">Manage commission type names</p>
            </div>
            <a href="{{ route('admin.commission-setting.create') }}"
        ...
        <div class="bg-white/80 rounded-2xl shadow-lg shadow-indigo-100/50 border border-white/50 overflow-hidden">
            <div class="p-5 border-b border-gray-100 flex items-center justify-between">
                <div class="flex items-center gap-2">
                    ...
                    <span class="font-semibold text-gray-700">All Commission Types</span>
                </div>
                <span class="text-xs text-gray-400 bg-gray-50 px-3 py-1 rounded-full">DataTable</span>
            </div>
            <div class="p-5 overflow-x-auto">
                <table id="commissionSettingTable" class="w-full">
                    <thead>
...
<script>
    $(document).ready(function () {
        $('#commissionSettingTable').DataTable({
            processing: true,
            serverSide: true,
            scrollX: true,
            autoWidth: false,
            ajax: "{{ route('admin.commission-setting.index') }}",
            columns: [
        ...
        $('#commissionSettingTable_length select').addClass('form-input !py-1.5 !px-3 !text-sm !rounded-xl !border-gray-200 !bg-gray-50 focus:!bg-white !transition-all !w-20');
        $('#commissionSettingTable_length label').addClass('text-sm text-gray-500 flex items-center gap-2');
    });
    function deleteItem(id) {
        Swal.fire({
            title: 'Are you sure?',
            text: "This commission type will be permanently deleted!",
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                fetch(`/admin/commission-setting/delete/${id}`, {
                    method: 'DELETE',
                    headers: {
                    ...
                    .then(data => {
                        if (data.success) {
                            ...
                            $('#commissionSettingTable').DataTable().ajax.reload(null, false);
                        } else {
                            Swal.fire({
                                icon: 'error',
                                title: 'Failed!',
