    ...
    <div class="max-w-4xl mx-auto">
        ...
        <div class="mb-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
                ...
                <p class="text-sm text-gray-500 mt-1 ml-13">Add a new commission type and rate</p>
            </div>
        ...
        <div class="bg-white/80 rounded-2xl shadow-lg shadow-indigo-100/50 border border-white/50 overflow-hidden">
            ...
            <div class="p-6">
                <form action="{{ route('admin.commission.create') }}" method="POST">
                    @csrf
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="flex flex-col">
                            ...
                            <select name="commission_type"
                                ...
                                <option value="{{ $type->name }}" {{ old('commission_type') == $type->name ? 'selected' : '' }}>{{ $type->name }}</option>
                                @endforeach
                            </select>
